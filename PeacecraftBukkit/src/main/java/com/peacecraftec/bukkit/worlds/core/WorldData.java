...
public class WorldData {
	...
	private ItemStack loadItem(String prefix, int slot) {
		if(!this.data.contains(prefix + ".data.slots." + slot)) {
			return null;
		}
		
		return ItemStack.deserialize(this.data.getMap(prefix + ".data.slots." + slot));
	}
	...
	private void saveItem(String prefix, int slot, ItemStack item) {
		...
		this.data.setValue(prefix + ".data.slots." + slot, item.serialize());
	}
