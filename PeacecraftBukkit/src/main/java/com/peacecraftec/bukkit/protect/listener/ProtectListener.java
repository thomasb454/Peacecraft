...
public class ProtectListener implements Listener {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		if(event.getAction() == Action.LEFT_CLICK_BLOCK || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
			...
			CommandSender sender = BukkitCommandSender.wrap(event.getPlayer(), this.module.getManager().getLanguageManager());
			if(action != null) {
				if(!PROTECTABLE.contains(event.getClickedBlock().getType())) {
					sender.sendMessage("protect.not-protectable");
					...
					if(!protection.getOwner().equals(event.getPlayer().getUniqueId().toString()) && !event.getPlayer().hasPermission(ProtectPermissions.MODIFY_ALL)) {
						...
						for(String player : ((AccessAddAction) action).getPlayers()) {
							...
							if(uuid == null) {
								sender.sendMessage("protect.unknown-player", player);
								return;
							}
						...
						sender.sendMessage("protect.added-allowed-players", build.toString());
						...
						sender.sendMessage("protect.removed-allowed-players", build.toString());
						...
						sender.sendMessage("protect.allowed-players", build.toString());
					} else {
						sender.sendMessage("protect.protection-already-exists");
					}
				} else {
					if(action instanceof ProtectAction) {
						...
						sender.sendMessage("protect.protected");
					} else {
						sender.sendMessage("protect.protection-doesnt-exist");
					}
				}
				...
				if(event.getAction() == Action.RIGHT_CLICK_BLOCK) {
					sender.sendMessage("protect.protected-by", this.module.getManager().getCasedUsername(this.module.getManager().getUsername(UUID.fromString(protection.getOwner()))));
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		if(PROTECTABLE.contains(event.getBlock().getType())) {
			...
			if(protection == null) {
				...
				BukkitCommandSender.wrap(event.getPlayer(), this.module.getManager().getLanguageManager()).sendMessage("protect.protected");
			}
		}
	}
